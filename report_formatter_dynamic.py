...
def build_stat_box(label, value_str, delta_str):
    inner_width = 12
    top = f"┏━ {label:<{inner_width-3}}┓"
    value_line = f"┃ {value_str:<{inner_width-1}}┃"
    delta_line = f"┃ {delta_str:<{inner_width-1}}┃"
    bottom = "┗" + "━" * (inner_width + 1) + "┛"
    return [top, value_line, delta_line, bottom]
...
def format_funnel_visual(client_data, project_data, metrics, header_stats=None):
    """
    Render funnel visual for a specific project.
    """
    header_stats = header_stats or {}
    funnel_config = project_data.get('funnel_structure', {}) or client_data.get('funnel_structure', {})
    ...
    project_name = project_data.get('project_name', client_data.get('name', 'Project'))
    fans_box = build_stat_box(
        "Fans",
        header_stats.get('fans_value', f"{capture_metrics.get('total_subscribers', 0):,}"),
        header_stats.get('fans_delta', "+0.0% →"),
    )
    earn_box = build_stat_box(
        "Earn",
        header_stats.get('earn_value', format_currency(metrics.get('monthly_revenue'))),
        header_stats.get('earn_delta', "+0.0% →"),
    )
    summary_box = "\n".join(f"{fans_box[i]}  {earn_box[i]}" for i in range(len(fans_box)))
    
    funnel = f"""📊 Weekly Funnel Dashboard
...
Overall: {((new_clients/total_reach*100) if total_reach > 0 else 0.0):.3f}%
Target: 0.01%
...
def generate_full_report(client_data, project_data, metrics, last_metrics=None):
    """
    Assemble complete report for a project, including header stats.
    """
    last_metrics = last_metrics or {}
    
    fans_total = metrics.get('fans_total', metrics.get('capture', {}).get('total_subscribers', 0))
    prev_fans = last_metrics.get('fans_total', fans_total)
    fans_delta = percent_delta(fans_total, prev_fans)
    
    revenue_total = metrics.get('monthly_revenue', 0)
    prev_revenue = last_metrics.get('monthly_revenue', revenue_total)
    revenue_delta = percent_delta(revenue_total, prev_revenue)
    
    header_stats = {
        'fans_value': f"{fans_total:,}",
        'fans_delta': format_delta(fans_delta),
        'earn_value': format_currency(revenue_total),
        'earn_delta': format_delta(revenue_delta),
    }
    
    sections = []
    sections.append(format_funnel_visual(client_data, project_data, metrics, header_stats).lstrip('\n'))
    sections.append(format_performance_analysis(metrics, last_metrics).lstrip('\n'))
    sections.append(format_bottleneck_section(metrics).lstrip('\n'))
    sections.append(format_whats_working(metrics).lstrip('\n'))
    needs_attention = format_needs_attention(metrics)
    if needs_attention:
        sections.append(needs_attention.lstrip('\n'))
    action_plan = format_action_plan_section(metrics)
    if action_plan:
...
Couldn't fetch:
{chr(10).join([f"• {e}" for e in metrics['errors'][:3]])}
